...
from vote.tasks import example_task  # Replace with your actual Celery task
...
class VoteTaskTest(TestCase):       
    @patch('vote.tasks.example_task.delay')  # Replace with your Celery task
    def test_celery_task_called(self, mock_delay):
        example_task.delay()  # Trigger the task
        mock_delay.assert_called_once()  # Verify it was called
...
class VoteViewTest(TestCase):
    ...
    def test_vote_list_view(self):
        ...
        self.assertTemplateUsed(response, 'vote/vote_list.html')  # Update template name if needed

def test_task_execution(self):
    result = example_task.apply().get()  # Executes the task synchronously
